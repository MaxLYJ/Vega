...
{{
    ...
    class {0} : public vega::script::entity_script
    {{
    public:
        constexpr explicit {0}(vega::game_entity::entity entity)
    ...
    private:
        ...
        bool Validate()
        {
            ...
            {
                errorMsg = "Please Type In A Script Name";
            }
            ...
            {
                errorMsg = "Please Select A Script Folder";
            }
            ...
            {
                errorMsg = $"Script With Name {name} Already Exists In This Folder!";
            }
            else
            {
                isValid = true;
            ...
            if(!isValid)
            {
                messageTextBlock.Foreground = FindResource("Editor.ColorBrush.Red") as Brush;
            }
            else
            {
                messageTextBlock.Foreground = FindResource("Editor.FontBrush.Normal") as Brush;
            }
        ...
        private void OnScriptName_TextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            if (!Validate()) return;
            var name = scriptName.Text.Trim();
            messageTextBlock.Text = $"{name}.h And {name}.cpp Will Be Added To {Project.Current.Path}";
        }
        ...
        private async void OnCreate_Button_Clicked(object sender, RoutedEventArgs e)
        {
            if (!Validate()) return;
            IsEnabled = false;
            ...
            catch (System.Exception ex)
            {
                ...
                Logger.Log(Utilities.MessageType.Error, $"Failed To Create Script {scriptName.Text}");
            }
            finally
            {
